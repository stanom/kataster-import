...
-- ###################################################### --
-- ### materializovaný náhľadu reg_C_vla_uzi - 3. nástrel
...
-- rýchly spôsob (14 minút) >>>
    ...
    ,string_agg(CASE WHEN "v".tuc=2 THEN E'[' || "v".vla::text || E' {' || "v".cit::text || E'/' || "v".men::text || E'}]' END , E'\n' ORDER BY "v".pcs::numeric) AS spravca
    ,string_agg(CASE WHEN "v".tuc=3 THEN E'[' || "v".vla::text || E' {' || "v".cit::text || E'/' || "v".men::text || E'}]' END , E'\n' ORDER BY "v".pcs::numeric) AS najomca
-- pomalý spôsob (niekoľko hodín) >>>
--    ,array_to_string(array(    SELECT pcs::text || E'. [' || vla::text || E' {' || cit::text || E'/' || men::text || E'}]' FROM kataster.kn_vl sub_v INNER JOIN ciselnik.tuc tuc ON(tuc.id=sub_v.tuc) WHERE (1=1 AND "sub_v".ku = p.ku AND "sub_v".clv = p.clv AND tuc.id=1) ), E'\r\n') AS vlastnici
    ,p.cel
    ,count(DISTINCT u.uzi::text) AS pocet_uzivatelov
...
--    ,p.parckey
    ...
    ,kl.gid
WITH NO DATA;
CREATE INDEX IF NOT EXISTS "idx_reg_C_vla_uzi_3_ku" ON kataster."reg_C_vla_uzi_3"(ku);
CREATE INDEX IF NOT EXISTS "idx_reg_C_vla_uzi_3_cislo_okresu" ON kataster."reg_C_vla_uzi_3"(cislo_okresu);
CREATE INDEX IF NOT EXISTS "idx_reg_C_vla_uzi_3_gid" ON kataster."reg_C_vla_uzi_3"(gid);
SELECT NOW();
REFRESH MATERIALIZED VIEW kataster."reg_C_vla_uzi_3";
SELECT NOW();
