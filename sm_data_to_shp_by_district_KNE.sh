...
### SKRIPT ###
	...
	ogr2ogr \
		--debug off \
                ...
                -sql "SELECT DISTINCT kn_c.gid ,ST_CollectionExtract(ST_MakeValid(kn_c.geom),3) AS geom ,utj.okres /*,COALESCE(utj.okres_nazov, ''::character varying) AS okres_nazov*/ ,kn_c.ku ,COALESCE(utj.ku_nazov, ''::character varying) AS ku_nazov ,COALESCE(kn_c.parcela, ''::character varying) AS parcela ,pa.cpa ,pa.ump ,pa.pkk ,pa.drp ,pa.vym ,pa.don ,pa.clv ,v.vla AS vlastnik ,v.cit ,v.men ,v.tuc ,v.pcs FROM kataster.kn_uov AS kn_c LEFT JOIN kataster.kn_ep pa ON pa.parckey::text = kn_c.parckey::text LEFT JOIN ciselnik.kataster utj ON kn_c.ku::numeric = utj.ku LEFT JOIN kataster.kn_vl v ON(pa.ku = v.ku AND pa.clv = v.clv) WHERE 1=1 AND kn_c.kmen > 0 AND kn_c.kmen IS NOT NULL AND kn_c.ku IN(SELECT DISTINCT ku FROM ciselnik.kataster WHERE 1=1 AND okres=${o}) ORDER BY cpa ,tuc ,pcs" \
               -nln "okres_${f}" \
               -nlt "MULTIPOLYGON" \
               -a_srs NULL \
               -lco ENCODING="UTF-8"
